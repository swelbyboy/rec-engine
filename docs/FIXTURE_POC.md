# Fixture PoC (original `/recommend` endpoint)

This documents the original candidate-recommendation demo: 30 hand-written
candidate fixtures, a weighted-linear scorer, and the `/recommend` /
`/recommend/upload` endpoints. It still runs (`store.py`, `scoring.py`,
`explanation.py`), but it's a separate, self-contained demo from the live
pipeline documented in the main [`README.md`](../README.md) — nothing here
is used by the Live PoC or the Mind comparison UI.

## ML / data science concepts

This PoC demonstrates a real ML pipeline that can be extended, not a keyword filter or UI wrapper. The core concepts:
...
Requires Python 3.11+ and the same `.venv` + `.env` setup as the main
README's step 1–2 (`ANTHROPIC_API_KEY` + `OPENAI_API_KEY` only — this demo
...
## Project structure (fixture-PoC-relevant files)
...
│   ├── extraction.py         # Claude tool-use extraction (JD + candidate) — SHARED with the live pipeline
│   ├── store.py              # Candidate store with content-hash caching
│   ├── retrieval.py          # OpenAI embedding retrieval (top-K by cosine sim)
│   ├── constraint_engine.py  # Canonical key + semantic constraint matching — SHARED with the live pipeline
